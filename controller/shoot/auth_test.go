	...
	Describe("mergeAuthenticationConfigurations", func() {
		...
		DescribeTable("should correctly merge authentication configurations",
			func(
				...
				Expect(initialGardenConfigMap.Data).To(HaveKey(authConfigKey))

				// Verify other data keys are preserved (keys that are not authConfigKey)
				...
				err = yaml.Unmarshal([]byte(initialGardenConfigMap.Data[authConfigKey]), &actualConfig)
				Expect(err).NotTo(HaveOccurred())

				// Unmarshal the expected configuration
				var expectedConfig apiserverv1beta1.AuthenticationConfiguration
				err = yaml.Unmarshal([]byte(expectedConfigMap.Data[authConfigKey]), &expectedConfig)
				Expect(err).NotTo(HaveOccurred())
			...
			Entry("with empty Garden ConfigMap and one Greenhouse issuer",
				...
				&corev1.ConfigMap{
					Data: map[string]string{
						authConfigKey: `apiVersion: apiserver.config.k8s.io/v1beta1
...
`,
		...
		It("should return error for invalid YAML in Garden ConfigMap", func() {
			configMap := &corev1.ConfigMap{
				Data: map[string]string{
					authConfigKey: "invalid: yaml: content: [",
				},
			}
